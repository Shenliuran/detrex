...
class FlowDETR(nn.Module):
    ...
    def __init__(
        ...
        # freeze pretrained resnet backbone for fine turning
        self.backbone.requires_grad_(not fine_turning)
        self.in_features = in_features
        self.position_embedding = position_embedding
        # freeze pretrained position embedding sine for fine turning
        # self.position_embedding.requires_grad_(fine_turning)

        # project the backbone output feature
        # into the required dim for transformer block
        self.input_proj = nn.Conv2d(in_channels, embed_dim, kernel_size=1)
        # freeze pretrained input flatten project for fine turing
        # self.input_proj.requires_grad_(False)

        # define learnable object queries and transformer module
        self.transformer = transformer
        self.query_embed = nn.Embedding(num_queries, embed_dim)
        # self.query_embed.requires_grad_(fine_turning)
