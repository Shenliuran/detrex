...
class LinearAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        # Allocation
        source_competition = torch.softmax(conserved_source / self.competition_temperature, dim=-1) * float(
            key.shape[2])
        ...
        #     # out_update = qk @ v
        #     out_update = ((qk @ (v * source_competition[:, :, :, None])) * sink_incoming[:, :, :,
        #                                                                    None]) * sink_allocation[:, :, :, None]
...
class LinearCrossAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        # (1) Calculate incoming and outgoing flow
        sink_incoming = 1.0 / (self.sum(q + self.eps, k + self.eps))
        source_outgoing = 1.0 / (self.sum(k + self.eps, q + self.eps))
