...
if __name__ == '__main__':
    attn = LinearAttention(256, 8, competition_temperature=1)
    # attn = LinearAttention(256, 8, competition_temperature=1)
    ...
    if attn_type == "cross":
        flow_attn_profiler = CrossAttentionProfiler(memory_len=3400, num_query=100, bs=16, embed_dim=256,
                                                    attn=attn,
                                                    use_cuda=True,
                                                    profile_memory=True, autograd=True,
        ...
        flow_attn_profiler = SelfAttentionProfiler(feature_len=3400, bs=16, embed_dim=256, attn=attn,
                                                   use_cuda=True,
                                                   profile_memory=False, autograd=True
                                                   # output_path="/root/autodl-tmp/profiler_output/flow_self_attention_profiler.json"
                                                   )
